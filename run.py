...
def main():
    ...
    if config.batch_size is None: 
        config.batch_size = max(1, int(40 / (config.prompt_size / 5)))
    
    mse = 0
    for i in tqdm(range(0, len(dataset), config.batch_size), total=int(len(dataset)/config.batch_size)):
        batch = dataset[i:i+config.batch_size]
        outputs = prompt_model(model, tokenizer, batch['prompt'], config)
        outputs = torch.tensor([int_or_nan(x) for x in outputs])
        targets = batch['answer']
        mse += evaluate(outputs, targets)
    mse /= len(dataset)
